import type { Lead } from "../normalizers/adf";

export type ClassifiedLead = Lead & {
  vendor?: string;
  subSource?: string;
...
export type SourceCtx = {
  ...
  providerUrl?: string;
  starSenderName?: string;
  starCreator?: string;
};

const N = (s?: string) => (s || "").toLowerCase();
const key = (s?: string) => { const k = N(s).replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, ""); return k || undefined; };
const any = (res: RegExp[], s?: string) => res.some(r => r.test(s || ""));

export function classifyLeadSource(base: Lead, ctx: SourceCtx): ClassifiedLead {
  ...
  const withTag = (vendor: string, subSource: string, channel: ClassifiedLead["channel"]) =>
    ({ ...base, vendor, subSource, channel });

  const matchOne = (hay: string, pairs: [string, string][]) => {
    const s = hay.toLowerCase();
    for (const [needle, value] of pairs) if (s.includes(needle)) return value;
    return undefined;
  };
